...
exports.helloWorld = (req, res) => { 
  ...
  const dishName = req.query.dish || req.body.dish || 'oysters';
  return getSentiments(restaurantAlias, dishName).then((sentiments) => res.send(
  {
    restaurant: restaurantAlias,
    dish: dishName,
    body: sentiments,
  }
  ));
...
async function getSentiments(restaurantAlias, dishName) {
  const sentiments = [];
  const reviews = await getReviews(restaurantAlias);
  console.log(reviews);
